import { Client as ZkDuelClient, type Game } from './bindings';
...
/**
 * Service for interacting with the ZkDuel game contract
 */
export class ZkDuelService {
  private baseClient: ZkDuelClient;
  private contractId: string;

  constructor(contractId: string) {
    this.contractId = contractId;
    // Base client for read-only operations
    this.baseClient = new ZkDuelClient({
  ...
  private createSigningClient(
    ...
    return new ZkDuelClient(options);
  }
  ...
  async prepareStartGame(
    ...
    const buildClient = new ZkDuelClient({
      contractId: this.contractId,
      networkPassphrase: NETWORK_PASSPHRASE,
      rpcUrl: RPC_URL,
  ...
  /**
   * Commit a move (hashed move + item + salt)
   */
  async commitMove(
    sessionId: number,
    ...
    try {
      ...
      if (err instanceof Error && err.message.includes('Transaction failed!')) {
        throw new Error('Transaction failed - check if the game is still active and you haven\'t already committed');
      }
      throw err;
    }
  }
  ...
  async revealMove(
    ...
    try {
      ...
      if (err instanceof Error && err.message.includes('Transaction failed!')) {
        throw new Error('Transaction failed - check if both players have committed');
      }
      throw err;
    }
  }
...
// Example: const zkDuelService = new ZkDuelService(contractId);
